<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.name" class="filter-item" placeholder="任务名" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.createBy" class="filter-item" placeholder="创建者" @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.state" class="filter-item" placeholder="请选择XX">
        <el-option
      ...
      <el-date-picker v-model="date" class="filter-item" type="date" placeholder="选择日期" />
    ...
    <div class="edit-container">
      ...
      <el-button size="small" type="danger" @click="deleteInBatches"><svg-icon icon-class="delete" />删除</el-button>
      <el-button size="small" type="info" icon="el-icon-refresh-right" @click="handleFilter">刷新</el-button>
      <div class="filter-button">
        <el-button v-waves size="small" type="primary" icon="el-icon-search" @click="handleFilter">查找</el-button>
        <el-button v-waves size="small" type="info" icon="el-icon-refresh" @click="resetListQuery">重置</el-button>
      </div>
    </div>
    <el-table
      :key="tableKey"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="warning"
            style="margin-left:10px"
            @click="dialogRoleFormVisible = true;authorize(scope.row)"
          >分配用户 </el-button>
          <el-button
            size="small"
            type="warning"
            style="margin-left:10px"
            @click="officeDialogFormVisible=true;authorize(scope.row)"
          >分配机构 </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { fetchRoleList, handleUpdateRole, createRole, deleteLogicalRole, addRoleUser, addRoleOffice, deleteRoleUser, fetchRoleInfoById } from '@/api/role'
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        enname: [{ required: true, message: '请输入角色编码', trigger: ['blur', 'change'] }],
        roleType: [{ required: true, message: '请输入角色类别', trigger: ['change', 'blur'] }],
        uacDepartment: [{ required: true, validator: validateDepart, trigger: 'change' }],
        uacActions: [{ required: true, validator: validateActions, trigger: 'change' }]
      },
  ...
  methods: {
    ...
    indexMethod1(index) {
      if (this.userRoleListQuery.page === 1) {
        ...
        return index + (this.userRoleListQuery.page - 1) * this.userRoleListQuery.size + 1
      }
    },
    indexMethod2(index) {
    ...
    getUserList() {
      fetchUserList(this.userListQuery).then(response => {
        ...
        this.allUserList = this.allUserList.filter(item => !Ids.includes(item.id))
    ...
    handleFilter() {
      ...
      if (this.listQuery.name === '' && this.listQuery.enname === '') {
        ...
        for (const v of this.list) {
          if (v.name.indexOf(this.listQuery.name) >= 0 && v.enname.indexOf(this.listQuery.enname) >= 0) { _list.push(v) }
        }
        if (!_list.length) {
          this.$message({
            message: '无该角色!',
    ...
    // 角色对应的用户中查找
    handleFilterUR() {
      if (this.userRoleListQuery.name === '' && this.userRoleListQuery.loginName === '' && this.userRoleListQuery.phone === '') {
        this.$message({
          message: '请输入信息!',
          type: 'warning'
        })
        ...
        for (const v of this.userList) {
          if (v.loginName.indexOf(this.userRoleListQuery.loginName) >= 0 && v.name.indexOf(this.userRoleListQuery.name) >= 0 && v.phone.indexOf(this.userRoleListQuery.phone) >= 0) { tempdata.push(v) }
        }
        if (!tempdata.length) {
          this.$message({
            message: '无该用户信息!',
    ...
    authorize(row) {
      this.roleId = row.id
      if (this.dialogRoleFormVisible) { this.getUserRoleList() } else {
        fetchOfficeById(this.roleId).then(res => {
          var tempdata = []
          if (res.data.list) {
    ...
    handleAuthorizeUsers() {
      ...
      // if (this.multipleSelection[i].uacRoles.indexOf(this.roleName) >= 0) {//查询数组中是否含有this.roleName,并返回索引
      //   //this.multipleSelection[i].uacRoles = this.multipleSelection[i].uacRoles.join(',')//数组中所有元素放入一个字符串,并以指定字符隔开
      // } else {
      //   this.multipleSelection[i].uacRoles.push(this.roleName)
      // }
      // const index = this.multipleSelection[i].uacRoles.indexOf(null)
      // if (index >= 0) {
      //   this.multipleSelection[i].uacRoles.splice(index, 1)
      // }
      // console.log(this.multipleSelection[i].uacRoles)
      // this.multipleSelection[i].roles = this.multipleSelection[i].uacRoles
      // updateUser(this.multipleSelection[i]).then(response => {
      //   this.$notify({
      //     title: '操作成功',
    ...
    cancleUserRole(row) {
      ...
      deleteRoleUser(role_user_delete).then(() => {
        this.$notify({
          title: '成功',
          message: '取消成功',
          type: 'success',
    ...
    handleUpdate(row) {
      ...
      fetchRoleInfoById(this.temp.id).then(res => {
        console.log(res.data)
      })
      ...
      this.dialogFormVisible = true
      this.$nextTick(() => {
        if (this.temp.uacActions) { this.$refs.tree.setCheckedKeys(this.temp.uacActions) } else { this.$refs.tree.setCheckedKeys([]) }
      })
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
    ...
    handleDelete(row) {
      ...
      deleteLogicalRole(this.temp.id).then(response => {
        this.$notify({
          title: '操作成功',
    ...
    deleteInBatches() {
      for (var v of this.multipleSelection) {
        deleteLogicalRole(v).then(response => {
          this.$notify({
            title: '操作成功',
            message: response.msg,
            type: 'success',
